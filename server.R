...
if (!require(slidify)){ 
  install_github('ramnathv/slidify', 'ramnathv', ref = 'dev', force = TRUE)
}

if (!require(rCharts)){ 
  install_github('ramnathv/rCharts', 'ramnathv',  force = TRUE)
}


if (!require(slidifyLibraries)){ 
  install_github('ramnathv/slidifyLibraries', 'ramnathv', ref = 'dev', force = TRUE) # optional
}
...
shinyServer(function(input, output) {
  ...
  output$summary <- renderPrint({
    ...
    ifall<-if(input$id3 !="") " AND " else "";
    term = c(paste(input$id1, ifall,input$id3), paste(input$id2, ifall,input$id3))
    getPubmedTrend(term, year=as.numeric(input$id4):input$id5)
  ...
  output$documentation <- renderUI({
   ...
   <b>Introduction of Comparing two Pubmed search results</b></br>
    ...
    The third tab is Total numbers and Ratio. It describe the results of total numbers and calculatied ratio of total numbers of the two searches.Please click the tab and Submit button and wait for a few seconds to see the result. ")
  })
})
